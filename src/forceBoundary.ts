...
type ArbitraryForce<IForce, NodeDatum extends SimulationNodeDatum> = IForce &
  Force<NodeDatum, any>
type ScalarAccessor = <NodeDatum extends SimulationNodeDatum>(
  node: NodeDatum,
  i: number,
  nodes: NodeDatum[]
) => number
...
export default function forceBoundary<NodeDatum extends SimulationNodeDatum>(
  ...
  function initialize(nodes: NodeDatum[]) {
    ...
    for (let i = 0; i < n; ++i) {
      _strengthsX[i] =
        isNaN((_x0z[i] = +_x0(nodes[i], i, nodes))) ||
        isNaN((_x1z[i] = +_x1(nodes[i], i, nodes)))
          ? 0
          : +_strength(nodes[i], i, nodes)
      _strengthsY[i] =
        isNaN((_y0z[i] = _y0(nodes[i], i, nodes))) ||
        isNaN((_y1z[i] = +_y1(nodes[i], i, nodes)))
          ? 0
          : +_strength(nodes[i], i, nodes)
  ...
  function strength(
    _newStrength?: ScalarAccessor | number
  ): ScalarAccessor | typeof force {
    if (!isUndefined(_newStrength)) {
      if (isNumber(_newStrength)) _strength = constant(_newStrength)
      else _strength = _newStrength
      return force
    }
    return _strength
  }
  ...
  function border(
    _newBorder?: ScalarAccessor | number
  ): ScalarAccessor | typeof force {
    if (!isUndefined(_newBorder)) {
      if (isNumber(_newBorder)) _border = constant(_newBorder)
      else _border = _newBorder
      return force
    }
    return _strength
  }
  ...
  const force = function force(alpha: number) {
    for (let i = 0, n = _nodes.length, node; i < n; ++i) {
      ...
      // debugger;
      if (
        node.x! < _x0z[i] + _borderz[i] ||
        node.x! > _x1z[i] - _borderz[i] ||
        node.y! < _y0z[i] + _borderz[i] ||
        node.y! > _y1z[i] - _borderz[i]
      ) {
        node.vx! += getVx(
          _halfX[i],
          node.x!,
